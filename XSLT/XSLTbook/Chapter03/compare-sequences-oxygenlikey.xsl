...
<!-- compare-sequences.xsl -->
	...
	<!-- this transformation runs, but not with the proper results. i might look at it again when i know xsl better -->
	...
	<!-- When I try to run the mark up from the book in Eclipse, I get an error saying the number sequence is bad syntax. I think has something to do with which version of Saxon Eclipse supports (it runs if I only have one number as the value of select) -->
	<!-- When I try to run the mark up from the book in Oxygen, it does not like xs:integer-->
	<!-- This is what the book has
			<xsl:variable name="testSequence1" as="xs:integer*" 
	 select="(8203, 22101, 27408, 19268, 14336)"/> -->
	<xsl:variable name="testSequence1" select="(8203, 22101, 27408, 19268, 14336)" />
	...
	<!-- In addition to the variables, I had to make a few tiny syntax changes from the book, probably because of using a different XSLT processor -->
	...
	<!-- In addition to the variables, I had to make a few tiny syntax changes from the book, probably because of using a different XSLT processor -->
	<xsl:variable name="testSequence2" select="(19268, 17, 95, 6, 42)" />
	...
	<xsl:variable name="testSequence3" select="('blue', 'white', '19268')"/>
	...
	<xsl:variable name="testSequence4" as="xs:yearMonthDuration*"
	  select="(xs:yearMonthDuration('P3Y8M')
		...
		<xsl:variable name="testSequence4" 
			select="('P3Y8M', 'P4Y8M', 'P2Y9M')"/>
	...
	<xsl:value-of select="$testSequence4 > (P4Y7M)"/>
